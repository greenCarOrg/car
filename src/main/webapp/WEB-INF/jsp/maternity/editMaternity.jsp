                    ...
                    function(msg){
                        ...
                        if(resultCode == 'FAIL'){
                            if(StrUtil.isNotEmpty(id)){
                                ...
                                if(result.id!=id){
                                    UIkit.notify({message: '该手机号码已被月嫂注册，请更换手机号码!', status: 'danger', timeout: 1000, pos: 'top-center'});
                                    $("#phone").val("");
                                }
                            }else {
                                UIkit.notify({message: '该手机号码已被月嫂注册，请更换手机号码!', status: 'danger', timeout: 1000, pos: 'top-center'});
...
<h3 class="heading_b uk-margin-bottom"><c:if test="${empty entity.id}">添加</c:if><c:if test="${not empty entity.id}">编辑</c:if>月嫂</h3>
<div class="md-card">
    <div class="md-card-content large-padding" style="height: auto !important;">
        <form:form id="validateSubmitForm" cssClass="uk-form-stacked" action="edit.do" commandName="maternityMatron" enctype="multipart/form-data">
                ...
                <div class="uk-width-medium-1-4">
                    <div class="parsley-row">
                        ...
                        <select name="type" id="type" style="width: 100%;">
                            <option value="0" <c:if test="${entity.type==0}">selected</c:if>>月嫂</option>
                            <option value="1" <c:if test="${entity.type==1}">selected</c:if>>育婴师</option>
                        </select>
                    </div>
                </div>
                <div class="uk-width-medium-1-4">
                    <div class="parsley-row">
                        <label for="name" class="uk-form-label required" >月嫂姓名</label>
            ...
            <div class="uk-grid" data-uk-grid-margin>
                <div class="uk-width-medium-1-4">
                    <div class="parsley-row">
                        <label class="uk-form-label">月嫂等级<span class="req">*</span></label>
                        <select name="level" id="level" style="width: 100%;">
                            <option value="1" <c:if test="${entity.level==1}">selected</c:if>>一星月嫂</option>
                            <option value="2" <c:if test="${entity.level==2}">selected</c:if>>二星月嫂</option>
                            <option value="3" <c:if test="${entity.level==3}">selected</c:if>>三星月嫂</option>
                            <option value="4" <c:if test="${entity.level==4}">selected</c:if>>四星月嫂</option>
                            <option value="5" <c:if test="${entity.level==5}">selected</c:if>>五星月嫂</option>
                            <option value="6" <c:if test="${entity.level==6}">selected</c:if>>特级月嫂</option>
                        </select>
                    </div>
                </div>
                <div class="uk-width-medium-1-4">
                    <div class="parsley-row">
                        ...
                        <input name="reputation" id="reputation" placeholder="输入0-5之间的数字" value="${entity.reputation}" onkeyup='this.value=this.value.replace(/\D/gi,"")' style="width: 50%;"/>
                    </div>
                </div>
                <div class="uk-width-medium-1-4">
                    <div class="parsley-row">
                        <label class="uk-form-label">状态<span class="req">*</span></label>
                        <select name="state" id="state" style="width: 100%;">
                            <option value="0" <c:if test="${entity.state==0}">selected</c:if>>未在职</option>
                            <option value="1" <c:if test="${entity.state==1}">selected</c:if>>已签约</option>
                        </select>
                    </div>
                </div>
                <div class="uk-width-medium-1-4">
                    <div class="parsley-row">
                        <label class="uk-form-label">服务价格(元/30天)<span class="req">*</span></label>
                        <input name="price" id="price" value="${entity.price}" style="width: 50%;"/>
                    </div>
                </div>
            </div>
            <div class="uk-grid" data-uk-grid-margin>
                <div class="uk-width-medium-1-4">
                    <div class="parsley-row">
                        <label class="uk-form-label">籍贯<span class="req">*</span></label>
                        <input name="nativePlace" id="nativePlace" value="${entity.nativePlace}" style="width: 50%;"/>
                    </div>
                </div>
            </div>
            <hr>
                ...
                <div class="uk-width-medium-1-5">
                    <div class="uk-form-row parsley-row">
                        ...
                        <span class="uploadImg">
                            <input type="file" name="isIdCardImg" id="isIdCardImg" class="myfiles" size="1" accept="image/*" onchange="PreviewImage(this,'isIdCardImgPreview',2)" multiple="multiple"/>
                            <a href="#">上传身份证图片</a>
                        </span>
            ...
            <div class="uk-grid" data-uk-grid-margin>
                ...
                <div class="uk-width-medium-1-3">
                    <div class="uk-form-row parsley-row">
                        <label>服务内容</label>
                        <span class="uploadImg">
                            <input type="file" name="serviceContent" id="serviceContent" class="myfiles" size="1" accept="image/*" onchange="PreviewImage(this,'serviceContentPreview',1)" />
                            <a href="#">等级认证证书图片</a>
                        </span>
                        <div id="serviceContentPreview" style="width:100%; height:100px;">
                            <c:if test="${not empty entity.serviceContentKey}">
                                <img src="${initParam['QNURL']}${entity.serviceContentKey}" style="width:180px;height:100px;"/>
                            </c:if>
                        </div>
                    </div>
                </div>
